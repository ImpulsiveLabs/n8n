...
const isRelevant = (
	...
	const keywords = (work.keywords || []).map((k: any) =>
		cleanText(k.display_name || '').toLowerCase(),
	);
	...
	for (const terms of Object.values(relevantTerms)) {
		if (
			terms.some((term) => Array.from(allText).some((text) => text.includes(term.toLowerCase())))
		) {
			matchingCategories++;
		}
	}
	...
	description: INodeTypeDescription = {
		...
		properties: [
			...
			},
			{
				...
				type: 'json',
				default:
					'{"management": ["management", "business", "organization", "strategy", "leadership", "administration", "enterprise"], "decision_making": ["decision", "decisions", "decision-making", "choice", "judgment", "planning"], "visualization": ["visualization", "visualizations", "data visualization", "visual", "graph", "chart", "dashboard", "analytics"]}',
			...
			},
			{
				...
				type: 'json',
				default:
					'["medicine", "medical", "surgery", "anesthesia", "fetal", "cancer", "disease", "biology", "clinical", "healthcare"]',
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				const relevantTermsRaw = this.getNodeParameter('relevantTerms', i, {}) as
					| string
					| Record<string, string[]>;
				const relevantTerms =
					typeof relevantTermsRaw === 'string' ? JSON.parse(relevantTermsRaw) : relevantTermsRaw;

				const excludeTermsRaw = this.getNodeParameter('excludeTerms', i, []) as string | string[];
				const excludeTerms =
					typeof excludeTermsRaw === 'string' ? JSON.parse(excludeTermsRaw) : excludeTermsRaw;
